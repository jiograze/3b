...
def plot_point_cloud(
    ...
    """
    Render a point cloud as a plot.
    ...
    :param fixed_bounds: if provided, use these bounds for all plots.
...
def plot_point_clouds(
    ...
    """
    Render multiple point clouds as subplots.

    :param pcs: list of PointClouds to plot.
    :param color: if True, show the RGB colors from the point clouds.
    :param grid_size: the number of random rotations to render for each cloud.
    :param fixed_bounds: if provided, use these bounds for all plots.
    :return: the matplotlib figure.
    """
    fig = plt.figure(figsize=(8 * len(pcs), 8))

    for idx, pc in enumerate(pcs):
        for i in range(grid_size):
            for j in range(grid_size):
                ax = fig.add_subplot(grid_size, grid_size * len(pcs), 
                                   1 + j + i * grid_size + idx * grid_size, 
                                   projection="3d")
                color_args = {}
                if color:
                    color_args["c"] = np.stack(
...
__all__ = ["plot_point_cloud", "plot_point_clouds"]
